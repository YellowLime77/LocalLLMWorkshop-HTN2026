...
export type ChatOptions = Omit<ChatCompletionCreateParamsNonStreaming, "model" | "messages">;
...
export const config: { cooldownMs: number; fallbackStatusCodes: Set<number>; localOptions: ChatOptions } = {
  cooldownMs: 60_000,
...
let clients: { cloud: OpenAI | null; local: OpenAI } | null = null;
let cloudRetryAt = 0;

function getClients(): { cloud: OpenAI | null; local: OpenAI } {
  if (!clients) {
...
function cloudIsDown(err: unknown): boolean {
...
export async function complete(messages: ChatCompletionMessageParam[], options: ChatOptions = {}): Promise<LLMResult> {
  ...
  if (cloud && Date.now() >= cloudRetryAt) {
    ...
    try {
      ...
      const why = err instanceof OpenAI.APIError ? (err.status ?? err.name) : String(err);
      console.warn(`[llm] Cloud failed (${why}). Using local Ollama for ${config.cooldownMs / 1000}s.`);
...
export async function chat(messages: ChatCompletionMessageParam[], options: ChatOptions = {}): Promise<string> {
